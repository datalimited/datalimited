...
pkg2_comsir_priors <- comsir_priors(ct = ct,
   ...
   mink = max(ct), maxk = max(ct) * 100, nsim = 1e4, logistic_model = TRUE)
})
...
identical(as.numeric(as.matrix(p_pkg)), as.numeric(as.matrix(p_orig)))
```
...
system.time({out <- DoProject(TruePar = as.data.frame(t(c(N1 = 800, K = 800, 
  ...
  Nsim = 1e5, Npost = 5e2, LogisticModel = TRUE, EstLogisticM = TRUE, start.r = c(0.2,1.0))})
